...
def _build_shared_msal_cache():
    ...
    try:
        if sys.platform == "win32":
            ...
            # On Linux the DataverseCLI uses libsecret (GNOME keyring) when a
            # keyring is available, but falls back to a *plaintext* cache file
            # in headless/container environments (it logs "Encrypted token
            # cache not available, falling back to plaintext"). Prefer that
            # plaintext file when it exists — libsecret is unavailable in such
            # environments and would yield zero accounts, forcing a needless
            # device-code prompt.
            plaintext_cache = (
                Path(os.environ.get("XDG_DATA_HOME", str(Path.home() / ".local" / "share")))
                / "Microsoft" / "DataverseCli" / "tokencache_msalv3.dat"
            )
            if plaintext_cache.exists():
                from msal_extensions import FilePersistence
                persistence = FilePersistence(str(plaintext_cache))
            else:
