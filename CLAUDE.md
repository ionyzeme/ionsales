...
> Publisher prefix for this project: **`ion_`** (solution: `ion_SalesPilot`).
> (Reuses the pre-existing `ion` publisher in the sandbox — chosen 2026-06-10 over a
> fresh `inz_` to match ionyze/ionsales branding.)
...
pac solution clone --name ion_SalesPilot # → YAML source tree (clean git diffs, modern flows/canvas)
...
3. **Codebase (L2):** edit the unpacked source in `ion_SalesPilot`; `pack` + `import`; verify.
...
- **Publisher + prefix on day one.** Create them before building anything; everything gets
  stamped (`ion_…`). This is what makes a clean managed export possible.
- **One unmanaged solution = unit of work AND unit of distribution.** Everything you build
  goes in `ion_SalesPilot`. **NEVER build in the Default solution** — leaked components
...
- **Managed is only ever an output.** Dev stays unmanaged forever:
  ```bash
  pac solution export --name ion_SalesPilot --managed       # from the dev source
  pac solution import --path ion_SalesPilot_managed.zip      # into a QA env (Sales+PO installed first)
  ```
...
├── solutions/
│   └── ion_SalesPilot/  (Layer 2 — pac solution clone YAML tree; the editable source)
...
3. **Create publisher + prefix** (`ion_`) and an **unmanaged solution** `ion_SalesPilot`
   in the portal.
4. **Connect MCP** in Claude Code; smoke-test ("show me the tables", "describe opportunity").
5. **`pac solution clone --name ion_SalesPilot`** into the repo; commit.
...
- Its entire definition lives in git (`solutions/ion_SalesPilot`), rebuildable via
